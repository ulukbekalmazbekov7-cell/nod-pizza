...
function complaintBranchName(complaint: Complaint) {
  return complaint.branches?.name ?? `Филиал #${complaint.branch_id}`;
}

function complaintSearchHaystack(complaint: Complaint) {
  return [
...
export default function TasksPage() {
  ...
  const filteredComplaints = useMemo(() => {
    ...
    return complaints.filter((complaint) => {
      ...
      if (query && !complaintSearchHaystack(complaint).includes(query)) return false;
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const synced = await syncComplaintWithJira(created.id);
      if (!synced) {
        await loadData();
      }
  ...
  return (
    <main className="min-h-screen bg-neutral-950 p-5 text-white md:p-6">
      <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold">Задачи / Жалобы</h1>
          <p className="mt-1 text-white/60">
            Регистрация обращений операторов и автоматическая постановка в Jira QC
          </p>
        </div>
        <button
          type="button"
      ...
      {showForm ? (
        <section className="mb-6 space-y-4 rounded-2xl border border-white/10 bg-neutral-950/50 p-4 md:p-5">
          <h2 className="text-xl font-semibold">Новая задача оператора</h2>
          ...
          >
            {saving ? "Сохранение…" : "Сохранить и отправить в Jira"}
          </button>
        </section>
      ) : null}
      ...
      {filteredComplaints.length === 0 ? (
        <EmptyState
          ...
          description="Измени фильтры или создай новую жалобу оператора."
        />
      ) : (
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h2 className="text-lg font-semibold">{complaintBranchName(complaint)}</h2>
